...
from chemes.client import ClientRequest, ClientResponse, ClientUpdate
from chemes.address import AddressRequest, AddressResponse
...
# Добавление клиента
@client_router.post("/clients/", response_model=ClientResponse, status_code=200)
...
@client_router.get("/clients/search", response_model=List[ClientResponse], status_code=200)
async def get_clients_by_name(name: Optional[str]= None, 
...
@client_router.delete("/clients/{client_id}", status_code=200)
...
@client_router.get("/clients/", response_model=List[ClientResponse], status_code=200)
